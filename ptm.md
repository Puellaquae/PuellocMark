...
块分为四类，容器快，独立块和内联块。内联块必须被包含在独立块或其他内联块内，内联块不能包含独立块。独立块只能包含内联块。内联块可递归包括内联块。
...
用于为所包裹内容追加属性，格式为 `<span` + 属性声明 + `>` + 内联块 + `</span>`，属性声明格式为 `attr=value` 或 `attr="value"` 和 `attr='value'`，多个声明使用空格隔开。
...
全局宏将应用于文档的每一块，一个文档只能有一个宏的全局使用块。
...
局部宏先于全局宏执行，层级深的语法宏先于层级浅的，同一层级位置前的先于位置后的。宏可以知道其他应用于此块及内部子块的宏的存在。
...
InlineBlock = InlineCode | Emphasis | Strong | Emoji | Link | Text | Macro | Image | Span
...
Span = "<span" SpanAttrs ">" InlineBlocks "</span>"

SpanAttrs = SpanAttr { " " SpanAttr }

SpanAttr = AttrName "=" ("\"" AttrValue "\"") | ("'" AttrValue "'") | AttrValue
