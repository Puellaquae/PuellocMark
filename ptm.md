...
宏位于一级标题前时，为全局使用（即无法将宏局部应用于一级标题。全局使用的效果等同于对每一个顶层块局部应用）。宏位于其他任意块前时，仅应用于紧跟那一块。文本宏只能全局应用或应用于顶级块。如果宏要应用于顶级块，则需位于块前一行，且行首不能为空白。
...
文本宏先于语法宏执行，全局先于局部，层级深的语法宏先于层级浅的，同一层级位置前的先于位置后的。

### 文本宏

文本宏接受一段源文本的输入（源文本可能包含其他宏使用），给出新的文本输出。源文本将用宏的输出替换。

### 语法宏

语法宏接受一个 AST 输入（输入的 AST 的内部的宏一定是已经处理了的），给出新的 AST。源 AST 将用宏的输出替换。

## EBNF

```ebnf
文档 = [ 元数据 ] { 顶层宏 } 一级标题 空行 { { 顶层宏 } 顶层块 空行 } 
